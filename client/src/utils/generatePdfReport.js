...
export const generatePdfReport = (record, patientProfile, user) => {
  ...
  doc.roundedRect(15, y, 180, 34, 3, 3, 'D');
  ...
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(5.5);
  doc.text('MEDICARE CLINIC', stampX, stampY - 7, { align: 'center' });
  doc.setFontSize(6.5);
  doc.text('★ VERIFIED ★', stampX, stampY - 2.5, { align: 'center' });
  doc.setFontSize(6);
  doc.text(`DR. ${doctorName.toUpperCase()}`, stampX, stampY + 2.5, { align: 'center' });
  doc.setFontSize(5);
  doc.setFont('Helvetica', 'normal');
  doc.text('OFFICIAL STAMP', stampX, stampY + 7, { align: 'center' });
  ...
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.text('ATTENDING PHYSICIAN SIGNATURE', sigX, sigY);
  ...
  const regNo = `MCI Reg. No: MCI-2026-${(record._id || '999').toString().slice(-6).toUpperCase()}`;
  doc.text(regNo, sigX, sigY + 20);
  doc.text(`${doctorHospital}`, sigX, sigY + 24);
  ...
  doc.setFontSize(7.5);
  doc.setFont('Helvetica', 'normal');
  doc.text('This report was digitally generated by the MediCare Healthcare Platform.', 105, pageHeight - 20, { align: 'center' });
  doc.text('Medical information in this report is based on the consultation record entered by the healthcare professional.', 105, pageHeight - 16, { align: 'center' });
  doc.text(`Document reference: ${appointmentId} | Digitally Signed & Stamped`, 105, pageHeight - 12, { align: 'center' });
